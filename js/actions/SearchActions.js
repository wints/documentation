...
var SearchActions = function() {
	return {
		...
		platformFromQuery: function(query, data) {
		...
		indexSource: function(term, data) {
			if (this.actions.platformFromQuery(term, data) == 'ios') {
				return [ data.indexes.ios, 'ios' ];
			}
			else if (this.actions.platformFromQuery(term, data) == 'android') {
		...
		search: function(term, data) {
			var dump = this.actions.indexSource(term, data);
		...
		top5: function(term, data) {
			var results = this.actions.search(term, data);
		...
		getResultOrigin: function(result) {
		...
		getContext: function(result, accuracy, query) {
